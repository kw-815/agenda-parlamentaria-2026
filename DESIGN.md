...
- **Filtro por eje:** los chips no ocultan filas individuales — ocultan la tarjeta del eje completo (con una transición de opacidad breve), porque ya no hay búsqueda de texto que deba filtrar dentro de una lista.
...
- **Componente:** reutiliza `.intro` (la misma tarjeta con fondo `--t-08`, borde translúcido y filo naranja a la izquierda) con un modificador `.intro--cierre`, en vez de crear un componente nuevo — es un bookend editorial deliberado: la misma pieza visual abre y cierra el reporte.
- **Etiqueta:** `.intro__label` — pequeña, mayúscula, color naranja explícito (no `var(--eje-color)`, porque esa variable solo existe dentro del scope de `.eje`; reusar `.eje__block-label` aquí habría sido inválido fuera de ese contexto).
...
### Fortalecimiento Institucional / Participación Ciudadana (`.section-mark`)
- **Eyebrow retirado (2026-08-13):** ambas secciones tenían un `.section-mark__eyebrow` (kicker en mayúsculas, ej. "Fortalecimiento institucional") arriba del `<h2>` — redundante con el propio `<h2>`, que ya dice lo mismo con más detalle. Se quitó el `<p>` en las dos secciones y la regla `.section-mark__eyebrow` (sin más usos en el proyecto).
- **Fix de ancho en el título de Participación Ciudadana:** el `<h2>` "Modernización de los Mecanismos de Participación Ciudadana" se veía cortado, con una línea final corta que no usaba el ancho disponible. Causa: `text-wrap: balance` (heredado de la regla global `h1,h2,h3,h4`) intenta igualar el largo de todas las líneas incluso cuando hay espacio de sobra — con un título de este largo, el balance dejaba una línea visualmente "desperdiciada". Fix: `.section-mark__title { text-wrap: pretty; }` — evita huérfanas sin forzar el balance estricto.
- **Nota de alcance retirada:** existía un `<p class="text-note">Solo títulos por ahora, sin el detalle de cada punto.</p>` bajo la lista de Participación Ciudadana — se quitó a pedido del usuario (2026-08-13), junto con la clase `.text-note` (sin más usos).
- **Tono editorial de `.section-mark__lead` (fix 2026-08-13):** el texto original describía a la Asamblea como un hecho ya cumplido ("Una Asamblea organizada y transparente...", "Mecanismos para una participación ciudadana más amplia..."), lo que sonaba a que Keyword estuviera haciendo una afirmación valorativa propia, o hablando como si fuera la Asamblea. Se reescribió con el mismo patrón de infinitivo-objetivo que ya usa `.eje__objetivo` ("Fortalecer...", "Ampliar y modernizar...") — reporta lo que la agenda se propone, no una afirmación sobre el estado actual. Aplica también a futuras ediciones: cualquier lead nuevo en `.section-mark` debe seguir ese mismo patrón de verbo-objetivo, no de descripción/juicio.
