...
- **User's explicit content scope for this pass** (2026-08-13): include all 3 ejes in full; from section IV include only sub-points 1 and 2 with their acciones estratégicas; from section VI include only the 3 titles (no bullet detail underneath). **Explicitly excluded for now:** section III (Fiscalización), section V (Diplomacia Parlamentaria), section VII (Metodología de Calificación) — not requested, do not add without asking.
...
- **Real photography integrated 2026-08-13**, per `IMAGENES/LEEME.md`: `hero.jpg` (hemiciclo de la Asamblea, hero background) and `eje-1/2/3.jpg` (background photo on each eje's filter-chip cover tile — see DESIGN.md "Chips" and "Fotos por eje"). One image (original `eje-1.jpg`) was rejected and replaced by the user after flagging it showed armed military at a checkpoint — against the guide's explicit "sin armas ni violencia explícita" rule for a corporate compliance/legal audience. A first placement of these photos as small corner thumbnails inside each `.eje` card was tried and reverted the same day after the user saw it live and rejected it as "diminuta en una esquina."
